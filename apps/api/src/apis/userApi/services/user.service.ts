...
class UserService {
	...
	async promoteToAdmin(
		...
		return User.findByIdAndUpdate(
			id,
			{ role: UserRole.ADMIN, subAdminId: adminId },
			{ new: true }
		).populate("avatar", "url");
	}
	...
	// SUPERADMIN: get all subadmins + users
	async getAllUsers(): Promise<IUser[] | null> {
		return User.find({ role: { $in: [UserRole.ADMIN, UserRole.USER] } }).populate(
			"avatar",
			"url"
		);
	}

	// SUBADMIN: get only users created under this subadmin
	async getAllUsersForAdmin(id: Types.ObjectId): Promise<IUser[] | null> {
		if (!Types.ObjectId.isValid(id)) throw new Error("Invalid ObjectId");
		return User.find({ subAdminId: id, role: UserRole.USER }).populate(
			"avatar",
			"url"
		);
	}
