...
export const createProductPackage = async (req: Request, res: Response) => {
  try {
    ...
    const { name, price, review, description, items, offers, usage, gender } =
      req.body;
...
export const getProductPackages = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user.role === "admin") {
      packages = await ProductPackageService.getAll(subAdminId);
      res.status(200).json({ success: true, data: packages });
    }
    const addedBy = req.user.subAdminId;
    packages = await ProductPackageService.getAll(
